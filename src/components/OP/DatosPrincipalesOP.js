...
export function DatosPrincipalesOP({ selectedOPData, idSociety }) {
  ...
  const { data: directionInfo } = useQuery(
    ['localización', selectedOPData?.localizacionId],
    () =>
      getMethod(`localizacion/mostrar/${idSociety.id}/${selectedOPData?.localizacionId}`)
  );
  ...
  const { mutate: setManagerInOP } = useMutation((newData) =>
  ...
  const rows = [
    {
      ...
      fechaInicio: selectedOPData?.fechaInicio,
      fechaFin: selectedOPData?.fechaFin,
