...
export function PageCobro({ mode, idSociety, loggedUser, fideicomisos, contrato}) {
  ...
  const [cuentas_destino, setCuentas_destino] = useState([]);  
  
  useEffect((idSociety) => {
    let id = idSociety?.id > 0? idSociety.id:JSON.parse(localStorage.getItem("idSociety"))?.id;
    getMethod(`contrato/listar/${id}/0`).then((cont) => setContratos(cont));
    getMethod(`cuentabanco/listarB/${id}/*`).then((cuenta) => setCuentas_destino(cuenta));
  }, []);
