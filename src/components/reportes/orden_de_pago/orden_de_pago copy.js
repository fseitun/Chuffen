...
const styles = StyleSheet.create({
  ...
  let fide_logo = 'http://localhost:3000/public/hornerorojo.png';
  fide_logo = './logo.png';
  fide_logo = 'http://localhost:3000/sociedades/1/logo_fide_1.png';
  const orden_de_pago = ({dataOP}) => (
            <Document >
                <Page size="A4" style={styles.page}>       

                    <View style={{flexDirection: 'row',marginLeft: '0',marginRight: '0'}} >
                        <Text style={{width: '60%'}}> </Text>
                        <View style={{width: '40%', backgroundColor: dataOP?.fideicomisos[0]?.color}} >
                            <Image style={[styles.logo, {backgroundColor: dataOP?.fideicomisos[0]?.color}]} src={{ uri: fide_logo, method: "GET", headers: { "Cache-Control": "no-cache" }, body: "" }} />                     
                        </View>
                    </View>
